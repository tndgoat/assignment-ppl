...
def main(argv):
    if len(argv) < 1:
        ...
        process1 = subprocess.Popen(
            ["java", "-jar", ANTLR_JAR, "ZCode.g4", "-Dlanguage=Java"],
            cwd="{}/test/gui".format(os.getcwd()),
        )
        process1.wait()

        process2 = subprocess.Popen(
            "javac -classpath {} ZCode*.java".format(ANTLR_JAR),
            cwd="{}/test/gui".format(os.getcwd()),
            shell=True,
        )
        process2.wait()
        ...
        process3 = subprocess.run(
            'java -cp ".{}{}" org.antlr.v4.gui.TestRig ZCode program -gui code.txt'.format(
                seperator, ANTLR_JAR
            ),
            cwd="{}/test/gui".format(os.getcwd()),
            shell=True,
