...
if (
    os.path.isdir("../target/main/zcode/parser")
    and not "../target/main/zcode/parser/" in sys.path
):
...
class TestCodeGen:
    ...
    def check(soldir, asttree, num):
        ...
        try:
            ...
            subprocess.call(
                "java  -jar " + JASMIN_JAR + " " + path + "/ZCodeClass.j",
                shell=True,
                stderr=subprocess.STDOUT,
            )

            subprocess.run(
                "java -cp ./lib;. ZCodeClass", shell=True, stdout=f, timeout=10
            )
        # except StaticError as e:
        #     f.write(str(e))
